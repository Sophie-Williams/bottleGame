	...
	class camera
		...
		public:
			...
			camera()
				{
				w = s = a = d = 0;
				position = position = XMFLOAT3(0, 0, -5);
				}
			void animation(bitmap *leveldata)//bitmap in the argument NOW YOU CAN USE IT HERE!!! *NEW*
				...
				if (w)
				{
					newpos.x -= forward.x * 0.2;
					newpos.y -= forward.y * 0.2;
					newpos.z -= forward.z * 0.2;
				}
				if (s)
				{
					newpos.x += forward.x * 0.1;
					newpos.y += forward.y * 0.1;
					newpos.z += forward.z * 0.1;
				}
				if (d)
				{
					newpos.x -= side.x * 0.1;
				...
				BYTE colorMatch = leveldata->get_pixel(-(newpos.x) / 2 + 50.5, -(newpos.z) / 2  + 0.5, 0);
				if (colorMatch < 10)
				{
					position = newpos;
				}
